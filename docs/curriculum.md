...
81. Explore a grid route breadth-first with a `std::queue` frontier.
82. Prioritize the lowest-cost route frontier with `std::priority_queue`.
83. Estimate remaining route cost with a heuristic.
84. Track a route distance while expanding a search.
85. Initialize unknown route cost safely with `std::numeric_limits`.
86. Combine a queue frontier with distance tracking for breadth-first routing.
87. Combine a priority frontier with distance tracking for Dijkstra-style routing.
