  ...
  if (message == WM_COMMAND) {
    switch (LOWORD(wparam)) {
      case 101: {
        ...
        const auto result = executor.compile_and_run({wide_to_utf8(source), {"lesson-31"}});
      ...
      case 103: application.start_lesson(31); refresh_snapshot(); SetWindowTextW(output_pane, text("ui.status.reset").c_str()); break;
    }
    return 0;
  }
...
int APIENTRY wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int command_show) {
  ...
  const auto lesson = application.campaign().find(31);
